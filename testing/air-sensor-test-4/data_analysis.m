d = importdata('IrnasOutdoor1.xlsx'); %import data
n = length(d.data); %number of data rows
n_mes = length(d.textdata(1,:)) - 2; %number of measurments

%Determine min and max values
min_mes = [-20,-20,-5,-20, -1000,-20,-20, 970,-20,-20, 970, -10, -10, -10,0,0,0,0,0,0,-10, -10, -10,0,0,0,0,0,0];
max_mes = [100, 100, 5, 100, 1000,70,100,1030,70,100,1030, 1000, 1000, 1000,0,0,0,0,0,0,1000, 1000, 1000,0,0,0,0,0,0 ];
log_scale = [false,false,false,false,false,false,false,false,false,false,false,true, true, true, false,false,false,false,false,false,true, true, true, false,false,false,false,false,false, false];
...
for i=1:n_mes
    ...
    for j=1:n_units
        ...
        s(i,j) = scatter((tt(j,start_n(i,j):end_n(j))-t_start)*24, squeeze(D(i,j,start_n(i,j):end_n(j))), 'MarkerEdgeColor', cc(j,:));
        ...
        s(i,j).Annotation.LegendInformation.IconDisplayStyle = 'off';
    end;
    ...
    %adjust limits
    lim = get(gca,'ylim');
    if(min_mes(i) ~= 0 &&  min_mes(i) > lim(1))
        lim(1) = min_mes(i);
    end;
    if(max_mes(i) ~= 0 &&  max_mes(i) < lim(2))
        lim(2) = max_mes(i);
    end;
    ylim([lim(1), lim(2)]);
