d = importdata('BioLab4a.xlsx'); %import data
...
for i=1:n_mes
    ...
    %plot all units
    for j=1:n_units
        scatter(tt(j,start_n(i,j):end_n(j)), squeeze(D(i,j,start_n(i,j):end_n(j))),'DisplayName',num2str(serial(j)), 'MarkerEdgeColor', cc(j,:));
        plot(tt(j,start_n(i,j):end_n(j)), squeeze(DA(i,j,start_n(i,j):end_n(j))),'DisplayName',num2str(serial(j)), 'color', cc(j,:),'LineWidth',1);
    end;
    
    %plot mean data
    plot(tt_mean(i,:),M(i,:),'r--','DisplayName','Mean','LineWidth',2.5);
